...
export namespace Aurion {
    ...
    bool ServiceLocator::RegisterService(Args&&... args)
    {
        ...
        auto [iter, success] = instance.services.try_emplace(key, std::make_unique<T>(std::forward<Args>(args)...));
    ...
    bool ServiceLocator::RegisterService(const T&& service)
    {
        ...
        // Only construct the object if the key doesn't exist in the map
        auto& instance = GetInstance();
        auto [iter, success] = instance.services.try_emplace(key, std::make_unique<T>(service));
    ...
    bool ServiceLocator::RestartService()
    {
        ...
        auto& instance = GetInstance();
        const auto iter = instance.services.find(key);

        // If not found, don't try to restart
        if (iter == instance.services.end())
        {
